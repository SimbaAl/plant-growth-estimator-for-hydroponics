...
X_for_training = features #This is our X input to SVM_model
...
classifier=svm.SVC()
random_search=RandomizedSearchCV(classifier,param_distributions=params,n_iter=3,n_jobs=-1,cv=3,verbose=3)
...
classifier=svm.SVC(kernel='rbf', random_state=1, gamma=0.01, C=30)
#classifier=svm.SVC(kernel='rbf', random_state=1, gamma=0.01, C=40)
...
SVM_model = classifier.fit(X_for_training,y_train)
timer(start_time) # timing ends here for "start_time" variable
...
test_for_SVM = np.reshape(X_test_feature, (x_test.shape[0], -1)) #reshaping the test features
